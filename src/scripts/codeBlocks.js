...
function setupCodeBlocks() {
  // 1) Enhance already-rendered wrappers (e.g., React-rendered CodeBlock)
  const wrappedBlocks = document.querySelectorAll(".code-block-wrapper");
  wrappedBlocks.forEach((wrapper) => {
    // Avoid rebinding
    if (wrapper._copyBound) return;

    const codeElement = wrapper.querySelector("pre code");
    const copyButton = wrapper.querySelector(".copy-button");
    const langLabel = wrapper.querySelector(".code-language");
    ...
    // If the wrapper lacks a language label, try to infer it
    if (codeElement && langLabel) {
    ...
    // mark as bound
    wrapper._copyBound = true;
  });

  // 2) For plain pre>code blocks (no wrapper yet), add wrapper + copy UI
  // Use broad selector for compatibility (avoid :has)
  ...
  codeBlocks.forEach((block) => {
    if (block.closest(".code-block-wrapper")) {
      return; // Already has wrapper; handled above
    }
